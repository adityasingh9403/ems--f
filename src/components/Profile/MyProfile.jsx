...
const MyProfile = () => {
    ...
    const handleSave = async () => {
        try {
            ...
            // --- YEH HAI FIX ---
            // Purani sessionStorage line ke bajaye naya function call karein
            updateUser(response.data);

            showToast('Profile updated successfully!');
            fetchData(); // Yeh local state refresh karega
            setIsEditing(false);
        } catch (error) {
    ...
    const handleFaceRegistered = async (descriptor) => {
        try {
            ...
            // 1. API call karein
            const response = await apiRegisterFace(descriptorArray);

            // 2. Backend se mila naya, clean user data
            const updatedProfile = response.data;

            // 3. YEH HAI SAHI FIX: Global state (AuthContext aur sessionStorage) ko update karein
            updateUser(updatedProfile);

            // 4. Is component ki local state ko bhi update karein
            // (fetchData() ki zaroorat nahi kyunki saara data response mein aa gaya hai)
            setCurrentUser(updatedProfile);
            setFormData(getInitialState(updatedProfile));
