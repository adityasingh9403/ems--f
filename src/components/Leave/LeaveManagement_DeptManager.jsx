...
const LeaveManagement_DeptManager = () => {
    const { user } = useAuth();
    ...
    const handleSaveLeave = async (leaveData) => {
        try {
            ...
            // Turant UI update ke liye fetchData call karein
            fetchData();
        } catch(error) {
    ...
    const teamRequests = allRequests.filter(r => r.requestorId !== user.id);
        ...
        <>
            ...
            <div className="bg-slate-50 dark:bg-slate-900 p-4 sm:p-6 space-y-6 min-h-screen">
                ...
                {loading ? <LoadingSpinner message="Loading leave requests..." /> : (
                    <>
                        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm fade-in-section">
                            <h2 className="text-xl font-semibold p-5 text-slate-800 dark:text-slate-100">Team's Requests</h2>
                                ...
                                <table className="w-full text-sm">
                                    <thead className="bg-slate-50 dark:bg-slate-700/50 text-xs uppercase">
                                        <tr>
                                            ...
                                            <th className="px-6 py-3 text-slate-600 dark:text-slate-300">Dates</th>
                                            <th className="px-6 py-3 text-slate-600 dark:text-slate-300">Status</th>
                                            <th className="px-6 py-3 text-slate-600 dark:text-slate-300">Action By</th>
                                            <th className="px-6 py-3 text-slate-600 dark:text-slate-300 text-center">Actions</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                                        {teamRequests.length > 0 ? teamRequests.map(req => (
                                            <tr key={req.id}>
                                                <td className="px-6 py-4 text-slate-800 dark:text-slate-200">{req.requestorName}</td>
                                                <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{req.startDate} to {req.endDate}</td>
                                                <td className="px-6 py-4">{getStatusBadge(req.status)}</td>
                                                <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{req.actionByName || '-'}</td>
                                                <td className="px-6 py-4 text-center">
                                                    {req.status === 'pending' ? (
                                                        <div className="flex items-center justify-center space-x-2">
                                                            <button onClick={() => handleStatusChange(req.id, 'approved')} title="Approve"><Check className="w-4 h-4 text-green-500" /></button>
                                                            <button onClick={() => handleStatusChange(req.id, 'rejected')} title="Reject"><X className="w-4 h-4 text-red-500" /></button>
                                                        </div>
                                                    ) : (
                                            ...
                                            <tr><td colSpan="5"><EmptyState icon={Users} title="No Pending Requests" message="No pending leave requests from your team."/></td></tr>
                                        )}
                                    </tbody>
                                </table>
                            </div>
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm fade-in-section">
                            ...
                            <div className="overflow-x-auto">
                                <table className="w-full text-sm">
                                    <thead className="bg-slate-50 dark:bg-slate-700/50 text-xs uppercase">
                                        <tr>
                                            <th className="px-6 py-3 text-slate-600 dark:text-slate-300">Leave Type</th>
                                            <th className="px-6 py-3 text-slate-600 dark:text-slate-300">Dates</th>
                                            <th className="px-6 py-3 text-slate-600 dark:text-slate-300">Status</th>
                                            <th className="px-6 py-3 text-slate-600 dark:text-slate-300">Action By</th>
                                        </tr>
                                    ...
                                    <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                                        {myRequests.length > 0 ? myRequests.map(req => (
                                            <tr key={req.id}>
                                                <td className="px-6 py-4 text-slate-800 dark:text-slate-200">{req.leaveType}</td>
                                                <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{req.startDate} to {req.endDate}</td>
                                                <td className="px-6 py-4">{getStatusBadge(req.status)}</td>
                                                <td className="px-6 py-4 text-slate-600 dark:text-slate-300">{req.actionByName || 'Pending HR/Admin Approval'}</td>
                                            </tr>
                                        )) : (
...
export default LeaveManagement_DeptManager;