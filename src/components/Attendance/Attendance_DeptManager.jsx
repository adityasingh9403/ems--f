...
const Attendance_DeptManager = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const teamMembers = employeesData.filter(emp => emp.departmentId === user.departmentId && emp.role === 'employee');
            setTeam(teamMembers);
    ...
    const teamRecordsForDate = team.map(teamMember => {
        if (isSelectedDateHoliday) return { ...teamMember, record: { status: 'holiday' } };
        
        const isOnLeave = approvedLeaves.some(l => 
            l.requestorId === teamMember.id && 
            selectedDate >= l.startDate && 
            selectedDate <= l.endDate
        );
        if (isOnLeave) return { ...teamMember, record: { status: 'on_leave' } };

        const record = allRecords.find(r => r.userId === teamMember.id && r.date === selectedDate);
        return { ...teamMember, record: record || { status: 'absent' } };
    });
        ...
        <>
            ...
            <div className="bg-slate-50 dark:bg-slate-900 p-4 sm:p-6 space-y-6 min-h-screen">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 fade-in-section">
                    <h1 className="text-3xl font-bold text-slate-800 dark:text-slate-100">Team Attendance</h1>
                    <button 
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 fade-in-section">
                    <h2 className="text-xl font-semibold text-slate-800 dark:text-slate-100 p-5">
                        Team Status for {new Date(selectedDate + 'T00:00:00Z').toLocaleDateString()}
                    </h2>
                            ...
                            {teamRecordsForDate.length > 0 ? (
                                <table className="w-full text-sm">
                                    ...
                                    <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                                        {teamRecordsForDate.map(member => (
                                            <tr key={member.id}>
                                                <td className="px-6 py-4 text-slate-800 dark:text-slate-200">{member.firstName} {member.lastName}</td>
                                                <td className="px-6 py-4">{getStatusBadge(member.record.status)}</td>
                                                <td className="px-6 py-4 text-slate-700 dark:text-slate-300">{member.record.clockIn ? new Date(member.record.clockIn).toLocaleTimeString() : '-'}</td>
                                                <td className="px-6 py-4 text-slate-700 dark:text-slate-300">{member.record.clockOut ? new Date(member.record.clockOut).toLocaleTimeString() : '-'}</td>
                                                <td className="px-6 py-4">
                                                    {member.record.clockInLocation && 
                                                        <a href={`https://www.google.com/maps?q=${member.record.clockInLocation}`} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline text-xs flex items-center gap-1">
                                ...
                                <EmptyState icon={Users} title="No Team Members" message="There are no employees assigned to your department yet." />
                            )}
                        </div>
                    )}
                </div>
...
export default Attendance_DeptManager;
